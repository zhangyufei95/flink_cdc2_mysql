...
public class FlinkHotTagCdc {
    public static void main(String[] args) throws Exception {
        ...
        Table tableQuery = tableEnv.sqlQuery("select * from (select puid,if(max(create_user_id)<776, 'algo', 'other') as tag_type,max(create_time) as create_time,max(release_date) as release_date,max(empty_city_id) as empty_city_id,max(empty_city_cluster) as empty_city_cluster,max(intention_city_id) as intention_city_id,max(intention_city_cluster) as intention_city_cluster,max(ban_push_time) as ban_push_time from (SELECT\n" +
                ...
                "WHERE\n" +
                "r.user_tag_id in (10001,10000,10002,10003) and r.status=1) tmp group by puid) ttmp\n" +
...
//                }
                ...
                //添加select 字段名
                projectColumns.add("intention_city_id"); //字段名
...
//                System.out.println(numRows);
                ...
                if ("7881302902169676".equals(puid)) {
                    ...
                    if (numRows > 0) {
                        while (results.hasNext()) {
                            ...
                            if (!(result.isNull(0))) {
                                ...
                                String intention_city_id1 = result.getString(0);
                                String intention_city_cluster1 = result.getString(1);
                                System.out.println("查出来的意向城市: "+intention_city_id1+"; "+intention_city_cluster1);
